...
*/
function whose_move2(_int){//used for turn info
	if(_int == 1){
		return document.getElementById("player1_id").value;
	}else{
		return document.getElementById("player2_id").value;
	}
}


//i got help for this check_winner() function
// I used it to find all the possible patterns for a winning tic tac toe board
//https://www.chegg.com/homework-help/questions-and-answers/javascript-please-help-implement-todo-js-file-html-question-4-tictactoe-part-assignment-ll-q57570169
...
function begin_play(){
	...
	if (isEmpty(p1) || isEmpty(p2)) {
		alert("Two player game, both fields are mandatory cannot start game yet");
		return;
	} 
	else { // we good the player names are filled so we disable them and change started to true
		if(started == false){
			document.getElementById('player1_id').disabled = true;
			document.getElementById('player2_id').disabled = true;
			this.started = true;
			document.getElementById("turn_info").innerHTML= " " + whose_move2(whose_move()) + "'s turn";
		}
		else
		{
			return;
...
function reset_play(){
	...
	document.getElementById('move_text_id').value = "Cell Number for Example: A1";
...
function play() {
	if (this.started) {
		var move = document.getElementById('move_text_id').value;// read in input
		if (!this.table_ids.includes(move)) { //check that move input is legal format
			alert("Invalid move check your answer format and try again");
		} else { //legal format good
			if(this.board_state[this.table_ids.indexOf(move)] == -1) //now check that the cell chosen isn't already filled
			{
				if (whose_move() == 1) { //player 1 sequence
					document.getElementById(move).innerHTML = "X";//update board
					this.board_state[this.table_ids.indexOf(move)] = 1; 
					document.getElementById("turn_info").innerHTML="Player "+ 2 + " turn";
					toggle_move();//change whose move
				} else { //p2 sequence
					document.getElementById(move).innerHTML = "O"; //update board
					this.board_state[this.table_ids.indexOf(move)] = 0;
					document.getElementById("turn_info").innerHTML="Player "+ 1 + " turn";
					toggle_move();//change whose move
				}
				check_winner();        
			}else{ // cell already has a move in it
