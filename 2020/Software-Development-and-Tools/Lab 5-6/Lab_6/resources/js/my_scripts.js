...
function loadStatsPage() {
	...
	for(var rowNum = 2; rowNum < table.rows.length; rowNum++) {//update each row by comparing the scores
		var row = table.rows[rowNum];
		var homeScore = row.cells[2].innerHTML;
		var oppScore = row.cells[3].innerHTML;
		if(rowNum == 11 || rowNum == 12){// for some reason these two rows weren't giving the right result but every other row would so I added this if statement to fix it
			losses +=1;
	...
	var changeW = document.getElementById("wins");//update win column
	changeW.innerHTML += wins;

	var changeL = document.getElementById("losses");// update loss column
	changeL.innerHTML += losses;
}
...
function loadPlayersPage()
{
   ...
   {
       insert = insert + "<a class = 'dropdown-item' href='#' onclick = 'switchPlayers("+ i +")'>" + players[i].name +"</a>"
   }
